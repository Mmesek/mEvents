...
def make_icon(name: str):
    return svg.Svg(role="img", viewBox="0 0 24 24", cls="h-6 w-6 mr-1")(
        svg.Path(d=SVG_BANK.get(name.lower()), fill="currentColor")
    )
...
PROVIDERS = {p: make_icon(p) for p in PROVIDER_LIST}
...
def login_buttons():
    return (
        fh.Div(
            ...
            *[mui.Card(provider_button(k, v)) for k, v in PROVIDERS.items()],
            mui.Card(magic_link()),
        ),
    )
